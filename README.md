...
- **Scalar metrics**: every metrics aggregation in a `requestType: "scalar"` builder query needs `reduceTo`. If the field is absent, the server looks up that metric in the spec's `source` and uses the same defaults as `signoz_query_metrics`: `sum` for monotonic counters; `avg` for gauges, non-monotonic sums, histograms, and exponential histograms. This includes hidden formula inputs. Explicit fields are preserved, including invalid values for upstream validation. Automatic discovery is limited to 16 distinct metric/source pairs and 30 seconds across the request. Set `reduceTo` explicitly to avoid metadata lookup; if metadata cannot identify a default or discovery reaches a limit, the tool returns recovery guidance. Applied reducers appear in the decisions note. Choose an explicit reducer for the intended statistic: summing rate buckets does not yield a total count, and averaging histogram percentiles does not yield a percentile over the full window. See `signoz://metrics-aggregation-guide`.
- **Order keys**: raw rows use `timestamp`; metric aggregates and formulas use `__result`. Log/trace aggregates use an aggregation expression such as `count()`, its alias, a zero-based index such as `"0"`, or a `groupBy` key. Query names such as `A`, `__result_0`, and `timestamp` are not generic aggregate order keys. Omit `order` for signal-aware defaults; explicit invalid keys are returned by upstream validation, with its valid-key guidance preserved.
