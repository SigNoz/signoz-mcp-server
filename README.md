...
- **Parameters**:
  ...
  - `requestType` (optional) - time_series (default) or scalar. Unknown values are rejected.
...
- **Parameters**:
  - `timeRange` (optional) - Time range like '2h', '6h', '2d', '7d' (default: 6h; ignored when both `start` and `end` are provided)
...
- **Parameters**:
  ...
  - `stepInterval` (optional) - Time bucket size in seconds for `time_series` mode (backend auto-selects when omitted)
