...
| Tool | Description |
|------|-------------|
| `signoz_get_org_overview` | Return an organization-scoped posture snapshot of dashboards, configured alerts/channels, views, log pipelines, and cloud integrations; deployment-wide signal/runtime stats are explicitly excluded |
...
Return a one-call organization posture snapshot before drilling into exact resources. The result is a stable grouped object with dashboard counts, configured alert-rule counts and dynamic type/signal breakdowns, notification-channel counts by type, saved-view counts by source, log-pipeline totals, cloud-integration connected-account counts, and completeness metadata.

- **Parameters**: no task parameters; like every tool, it accepts non-required `searchContext` for MCP observability.
- **Missing fields**: an omitted count means the upstream collector did not report it; it must not be interpreted as zero. The upstream endpoint can return a partial HTTP 200 when an individual collector fails. A missing group total produces `available: false`; a reported zero produces `available: true`. Expected missing or invalid fields set `metadata.partial` and add an `incompleteGroups` entry with affected output paths, reason, next action, and exact fallback tools where available. Complete absence of both cloud-provider stats is represented by `cloudIntegrations.sourceAvailability: "unavailable"` without degrading the rest of the snapshot because the backend cannot distinguish an unsupported/edition-gated feature from both provider queries failing.
- **Tenant scope**: `signals.available` and `alerts.runtime.available` are currently `false`. The upstream telemetry/infra and alert-runtime collectors are deployment-wide rather than organization-scoped, so this tool deliberately excludes their values. Their `nextTools` point to `signoz_search_logs`, `signoz_search_traces`, `signoz_list_metrics`, or `signoz_list_alerts` for tenant-scoped checks.
...
- **Cloud-integration completeness**: `cloudIntegrations.sourceAvailability` reports whether both current AWS/Azure provider counts were reported. Each provider has its own `dataAvailable`; an absent provider count is never converted to zero. If neither provider is reported, source availability is `unavailable` without setting overall `metadata.partial`; this can mean cloud integrations are unsupported/edition-gated **or** both provider queries failed, so do not interpret it as “no cloud integrations configured.” If exactly one is reported, source availability is `partial` and the missing provider receives non-recursive recovery guidance. `connectedAccounts` means a non-removed account with an account ID and at least one agent report, not a recent check-in or proof that an integration service is enabled.
- **Completeness metadata**: `reportedStatCount` is the size of the upstream stats bag; `includedStatCount` is the organization-scoped subset; `omittedStatCount` includes deployment-wide, unrelated, and invalid known stats; `excludedDeploymentWideStatCount` identifies the unsafe signal/runtime fields withheld; and `additionalStatCount` counts only compatible new organization-scoped observability fields preserved under `additionalStats`. Malformed known counters are excluded from `additionalStats` and named under `invalidStatFields`.

> **Requires [SigNoz ≥ v0.129.0](https://github.com/SigNoz/signoz/commit/287b60cbe61d06490bc1a4ddda6fe28002daf431)**, the first release with the organization stats endpoint.
