...
| `SIGNOZ_TENANT_URL_ALLOWLIST` | Multi-tenant (http) only: comma-separated allowlist of SigNoz backend hosts the server will proxy to. Entries are exact hosts (`signoz.example.com`) or wildcards (`*.us.signoz.cloud`, which matches any subdomain ending in `.us.signoz.cloud`). When set, client-supplied tenant URLs (OAuth setup form and `X-SigNoz-URL` header) that do not match are rejected with HTTP 403 and a `disallowed_signoz_url` auth-failure. Empty/unset allows any host. The operator's own `SIGNOZ_URL` is exempt. | No |
