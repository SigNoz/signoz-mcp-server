...
// the runtime default untouched and never errors.
//
// A soft limit bounds steady-state growth and reclaimable churn. It cannot
// shrink live data: the docs index build holds every analysed page in one
// bleve batch, and that peak is addressed separately by chunked indexing.
package memlimit
...
const (
	// defaultRatio is the fraction of the container memory limit used as the
	// soft GOMEMLIMIT, leaving headroom for non-heap memory (goroutine stacks,
	// runtime overhead) so the GC target stays below the hard cgroup limit.
	defaultRatio = 0.9

	// minLimitBytes guards against pathologically small or misread limits that
	// would cause GC thrashing; below this the soft limit is left unset.
	minLimitBytes = 64 << 20

	// v1UnlimitedThreshold: cgroup v1 reports "unlimited" as a near-max value
	// (for example 0x7FFFFFFFFFFFF000). Anything this large means no limit.
	...
	// RatioEnv overrides defaultRatio; values outside (0, 1] are ignored.
...
// readMemoryLimitFrom returns the container memory limit in bytes from cgroup
// v2, then v1, or ok=false if neither yields a finite limit. It reads the
// standard root cgroup paths, which are correct for the common containerized
// case (private cgroup namespace); anything else fails open.
func readMemoryLimitFrom(v2Path, v1Path string) (uint64, bool) {
	// cgroup v2 takes precedence; if its file exists, do not consult v1.
...
// parseMemLimitV1 parses cgroup v1 memory.limit_in_bytes; near-max values are
