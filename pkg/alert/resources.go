...
**Before using any example:** channel displayName values are illustrative. Reuse a fully paginated signoz_list_notification_channels result only from the same still-current prepared operation; otherwise call it and replace every direct-routing value with an exact returned displayName. If none fits, ask the user or offer signoz_create_notification_channel with user-provided config.kind/spec; never create automatically. Channel creation defaults test to false. Confirmed v2 policy routing may remove tier channels and set notificationSettings.usePolicy=true. Anomaly examples require direct top-level preferredChannels and cannot use policy routing.
