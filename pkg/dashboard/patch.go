...
- spec.panels is a MAP keyed by panel id: {"<panelId>": {"kind":"Panel","spec":{...}}}.
- spec.layouts is an ARRAY of Grid envelopes: [{"kind":"Grid","spec":{"items":[ {"x","y","width","height","content":{"$ref":"#/spec/panels/<panelId>"}} ]}}]. The grid item positions a panel; its content.$ref links it to the panels map. New dashboards already have one Grid at layouts[0]; grid items are 0-based in spec.layouts[0].spec.items.
...
- A panel holds exactly ONE query (backend-enforced). To combine series, replace the single query with a signoz/CompositeQuery — do NOT append a second entry to /spec/panels/<panelId>/spec/queries.
