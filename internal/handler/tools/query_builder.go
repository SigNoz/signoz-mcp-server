	...
	executeQuery := mcp.NewTool("signoz_execute_builder_query",
		...
		mcp.WithDescription(
			"Execute a SigNoz Query Builder v5 query.\n\n"+
				...
				"The query type discriminator lives on each query envelope, not the composite. "+
				"Set compositeQuery.queries[i].type to one of: \"builder_query\" (signal-specific spec), "+
				"\"builder_formula\" (formula expression), \"builder_trace_operator\" (trace operator spec), "+
				"\"promql\" (spec: {name, query, disabled, step?, legend?}), "+
				"or \"clickhouse_sql\" (spec: {name, query, disabled, legend?}). "+
				"Do NOT put a queryType field on compositeQuery itself — the backend rejects it.\n\n"+
				"For PromQL queries ALSO read signoz://promql/instructions — "+
