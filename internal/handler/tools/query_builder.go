	...
	executeQuery := mcp.NewTool("signoz_execute_builder_query",
		...
		mcp.WithDescription(
			"Use this only when dedicated log, trace, and metric tools cannot express the request, including multi-query requests, formulas, PromQL, and ClickHouse SQL. "+
				...
				"requestType \"heatmap\" returns a bucketed distribution from exactly one enabled metrics builder query, formula, PromQL, or ClickHouse query; bucketOptions applies only to that builder query or formula, not PromQL or ClickHouse. "+
