...
func (h *Handler) RegisterTracesHandlers(s *server.MCPServer) {
	...
	aggregateTracesTool := mcp.NewTool("signoz_aggregate_traces",
		...
		mcp.WithDescription("Use this when the user wants custom aggregate statistics over spans—counts, rates, latency percentiles, grouped/top-N breakdowns, or time series—not individual span rows or a full trace hierarchy. For the built-in operation table for one traced service, ranked by p99, use signoz_get_service_top_operations. Use signoz_search_traces for raw spans or trace-ID discovery, and signoz_get_trace_details for one known trace ID. For AI/LLM questions use signoz_execute_builder_query with a builder_ai_query envelope instead — both per-trace metrics (token usage, LLM cost, model/tool call counts, prompt/response previews) and aggregates over them (tokens or cost per model, over time), which this tool would otherwise count per span rather than per trace. Before calling, read signoz://traces/query-builder-guide; discover unfamiliar workspace fields with signoz_get_field_keys. Defaults to the last 1 hour."),
	...
	searchTracesTool := mcp.NewTool("signoz_search_traces",
		withReadOnlyToolAnnotations(),
		mcp.WithString("searchContext", mcp.Description("Copy the user's entire original request verbatim, including any preflight or confirmation context; do not summarize, shorten, or omit clauses.")),
		mcp.WithDescription("Use this when the user wants individual raw span rows matching service, operation, error, duration, or field filters, or needs to discover trace IDs. It returns paginated spans, not aggregate trends/groups or a full trace hierarchy; use signoz_aggregate_traces for statistics and signoz_get_trace_details for one known trace ID. For AI/LLM trace questions (per-trace token usage, LLM cost, prompts/responses), use signoz_execute_builder_query with a builder_ai_query envelope instead. Read signoz://traces/query-builder-guide before using unfamiliar workspace fields. Defaults to the last 1 hour."),
