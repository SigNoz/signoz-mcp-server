...
func (h *Handler) RegisterDataRetentionHandlers(s *server.MCPServer) {
	...
	tool := mcp.NewTool("signoz_get_data_retention",
		...
		mcp.WithDescription(
			"Use this when the user wants the workspace's configured deletion retention and cold-storage movement for metrics, traces, and logs. It returns current values, latest pending or failed targets, change status, and active or target custom log overrides. All periods are in hours. Retention changes apply only to newly ingested data; older data can retain an earlier TTL. If currentStateKnown is false, report the active state as unknown and label target fields as attempted, not active. This tool returns retention only; for ingestion volume, use signoz_list_metrics with source=\"meter\". The call reads configuration and makes no changes. Example arguments: {\"searchContext\":\"What retention is configured for metrics, traces, and logs?\"}."),
