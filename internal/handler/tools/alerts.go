...
func (h *Handler) RegisterAlertsHandlers(s *server.MCPServer) {
	...
	alertsTool := mcp.NewTool("signoz_list_alerts",
		...
		mcp.WithBoolean("active", mcp.Description("Include active (firing) alerts. Default: true (server-side). Accepts a boolean or the strings \"true\"/\"false\"."), boolOrStringType()),
		mcp.WithBoolean("silenced", mcp.Description("Include silenced alerts. Default: true (server-side). Accepts a boolean or the strings \"true\"/\"false\"."), boolOrStringType()),
		mcp.WithBoolean("inhibited", mcp.Description("Include inhibited alerts. Default: true (server-side). Accepts a boolean or the strings \"true\"/\"false\"."), boolOrStringType()),
