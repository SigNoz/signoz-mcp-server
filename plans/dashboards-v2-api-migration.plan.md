...
**Old types.** `pkg/types/dashboard.go` deleted; `QueryType` (used by `alertrule.go`) preserved in new `pkg/types/querytype.go`. `pkg/dashboard` resources are still used (schemas, the v6-adapted `Basics` basics guide + `WidgetsInstructions` concepts guide); the `dashboardbuilder`/`panelbuilder` pipeline (`dashboard.Validate`/`ValidateFromMap`) is now **unused** (import became pass-through too) and annotated `Deprecated` (both packages + `Validate`), retained pending removal (~6.8k lines). B's `validateFilterExpressionConsistency` not ported (v1-only two-filter hazard; v2 has a single `filter.expression`).
