...
- **Patch** — forward the RFC 6902 op array to `PATCH …/{id}`. Inject `webUrl` (known id) + `structuredResult`.
- **Get/Delete** — id via `readResourceID`; get injects `webUrl` + `structuredResult`; delete is 204 → confirmation text.
- **List** — `paginate.ParseParamsClamped` → `ListDashboards(ctx,limit,offset)` (limit clamped to `MaxLimit`; v2 also bounds server-side; clamp note surfaced); `util.InjectListWebURL` injects `webUrl` per `dashboards[]` entry, precision-preserving and handling the `{"data":{…}}` envelope; `structuredResult`.
