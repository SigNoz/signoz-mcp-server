...
- `santhosh-tekuri/jsonschema/v6` moves from indirect to direct dependency (the validation decorator imports it).
...
### C. Input schema validation — always on, never rejects, in-band notice (final design 2026-07-11)
Principle constraint (agent-first): tools accept flexible input types and normalize internally; a call an agent makes "wrong" that we can normalize is a contract defect on our side, not the model's. End users reach this server through heterogeneous agents, most of which do not self-correct from rejection errors — so validation informs (telemetry + in-band notice) and never gates.

- **No configuration.** `MCP_INPUT_VALIDATION_MODE` was dropped entirely (owner decision 2026-07-11, superseding the 2026-07-10/11 tri-state designs): validation always runs, never rejects, and needs no kill switch because every layer fails open (compile failure → undecorated tool; mismatch → call served). Identical behavior on HTTP and stdio.
- **Registration-time design**: the `h.addTool` wrapper (`schema_compat.go`) — the one place that sees the final normalized schema before SDK registration — compiles the schema once and decorates the tool's handler. Per call, validate the arguments; on mismatch emit a deduplicated WARN + `mcp.tool.validation.mismatches` metric, **proceed to the handler regardless**, and append an in-band `Input validation notice:` text block to the successful result naming the mismatched path and expected type. Self-correcting agents adjust on the next call; agents that never read errors still get a served result. Mismatches on real traffic drive schema fixes (union types, alias declaration), per "contract as the defect".
- **Dependency plumbing (explicit design)**: the decorator lives on `Handler` (`internal/handler/tools/handler.go`), which already owns the logger and meters — no package-level mutable configuration. `addTool` is the method `h.addTool` (all ~41 registration sites). Schemas are compiled after `normalizeToolSchemas`, and the compiled input/output schemas are closed over in the decorated handler. **Registration-time compile failure**: register the tool fail-open (undecorated) but emit a bounded startup ERROR log + metric; the B2 CI compile/inventory test independently fails the build.
- **No rejection path exists.** Design history: 2026-07-10 shipped a tri-state (off/shadow/enforce) with decorator-owned enforce rejections on the coded-error contract; 2026-07-11 the owner first dropped the default-flip goal (heterogeneous end-user agents mostly don't self-correct from rejections), then dropped the env var and enforce/off entirely in favor of always-validate + in-band notice. Rejection stays reserved for unrecoverable intent, which handlers already implement per-field (missing required param, unusable value).
...
- **Notice quality**: the appended notice must name the offending param and expected type (road to recovery); the decorator surfaces the flattened jsonschema error (`at '/limit': got boolean, want string`-style) with soft wording — the schema layer cannot know whether the handler ignored, defaulted, or normalized the value — pinned by test.
...
### D. Output schemas — enumerated read-only allowlist, decorator-validated (telemetry-only)
**Validation mechanism**: output validation for allowlisted tools happens inside the same registration-time handler decorator and is telemetry-only — an output mismatch is our defect, not the caller's, so the original result always passes through (WARN + `mcp.tool.validation.mismatches` with `validation.direction=output`). The decorator is the **only** output validator (`server.WithOutputSchemaValidation()` is not used). Nil-`StructuredContent`-on-success is a deduplicated WARN + counter (fail open, never silent). Success shapes keep arrays non-nil (`results`, `available_headings`) — the generated schema types Go slices as `["null","array"]` so `null` is technically schema-valid, but `[]` is the friendlier contract for typed clients, and the allowlist test validates every success against the advertised schema.
...
- ~~Enforce-mode default flip~~ then ~~the whole `MCP_INPUT_VALIDATION_MODE` tri-state~~ **dropped 2026-07-11** (see C): validation is always on and never rejects; mismatch telemetry + in-band notices drive normalization/widening fixes.
...
- **Accept-all-and-normalize**: always-on, never-rejecting validation (C) and the dual string/number guard (F) exist specifically so no currently-normalized agent input starts failing.
- **Contract is the defect, not the model**: validation mismatches are treated as schema bugs to fix (unions, alias declaration), not client errors.
- **Road to recovery in errors**: the in-band validation notice names the param and expected type so self-correcting agents can fix the next call without a failed turn.
...
2. **Validation notices must not fire on accepted forms**: params advertised as a single type but accepting alternate forms in handlers would emit spurious notices. Union types `{"type":["string","integer"]}` verified working in jsonschema/v6; every such param must declare the union. B1 catches drift; the replay test asserts zero notices on all accepted forms; the mismatch metric catches real-traffic unknowns.
...
10. **Client compat (in-band notice)**: the appended `Input validation notice:` text block is additive to the result content — clients reading only `content[0]` or `StructuredContent` are unaffected; text-concatenating clients see the notice after the result. No error shape changes.
...
- `internal/config/config.go` — no changes (the earlier `MCP_INPUT_VALIDATION_MODE` was removed 2026-07-11).
- `internal/handler/tools/handler.go` — validation mode field on `Handler` (owns logger/meters already).
- `internal/handler/tools/schema_compat.go` — `addTool` → `h.addTool` method (update ~41 registration sites); registration-time schema compile + always-on validation decoration with in-band input notice.
...
- Docs sync per CLAUDE.md checklist: README (validation-notice behavior note), `manifest.json`, `docs/` — checked and outcome stated in PR summary. agent-skills: no taught contract changes → expected no companion PR; state outcome.
...
- **Live smoke via subagent per CLAUDE.md**: copy an existing resource's shape rather than hand-crafting; never print or persist credentials; delete every created resource and confirm deletion; report which fields round-tripped. Exercise valid + deliberately invalid args on a representative tool set; confirm mismatched calls are served with an appended validation notice (never rejected) and the mismatch metric fires; confirm the notice text is LLM-actionable.
