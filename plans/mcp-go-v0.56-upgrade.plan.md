...
- **Enforce mode** (follow-up default-flip, wiring ships now): adds `server.WithInputSchemaValidation()` at construction; invalid args become SEP-1303 `CallToolResult{IsError:true}` so the model self-corrects in-band. Flip only after ~zero mismatch rate over a soak period on staging + production.
- **Authoritative final-outcome telemetry** (input rejections bypass tool middleware entirely — no `execute_tool` span, tool metric, or analytics event): a dedicated validation-rejection event (WARN log + metric with tool name and normalized param path) is the *authoritative* record for enforce-mode rejections; we explicitly accept that these calls do not produce `execute_tool` spans. Tests must prove **exactly one correctly classified terminal event** for each scenario: input rejection, output rejection, normal success, handler error.
- **Enforce-mode rejection detection rule**: `AfterCallTool` receives only the final `CallToolResult`; it recognizes input rejections by the SDK-pinned message prefix `input schema validation failed: ` (constant in mcp-go `server/output_validation.go`; `output schema validation failed: ` for output). Extraction of the param path is bounded: first line only, length-capped, values stripped. A contract test pins the prefix against actual SDK behavior so upstream message drift fails CI instead of silently blinding the detector.
- **Telemetry hygiene**: metric attributes use normalized, bounded param paths (strip array indexes and user-controlled property names; cap length). Logs carry bounded path/keyword metadata only — **never raw argument values** (tool inputs can contain webhook passwords, API keys).
- **Error-quality gate before enforce**: SEP-1303 rejection text must name the offending param and constraint (road to recovery — v0.56.0's readable `ErrorKind` fix helps); compare against current handler errors and augment if less actionable.
- Do **not** enable `WithStrictInputSchemaDefault()` — `additionalProperties:false` would break the silent `query` alias.
- Alignment note: SDK validation runs against the exact advertised schema — validator and published contract share one source, closing the advertised-schema-vs-handler-acceptance drift class.

### D. Output schemas — enumerated read-only allowlist, decorator-enforced
**Enforcement mechanism (fixes the double-telemetry problem)**: output validation for allowlisted tools happens inside our registration-time handler decorator, *before* the result returns to `loggingMiddleware` — on mismatch the decorator returns an `IsError` validation result, so the existing middleware records exactly one failure event. `server.WithOutputSchemaValidation()` stays enabled as defense-in-depth only (verified: the SDK validator skips results already marked `IsError`, nil `StructuredContent`, and error results). Nil-`StructuredContent`-on-success detection lives in the same decorator (bounded WARN + counter) — `structuredResult` falls back to text-only when upstream JSON fails to decode, which the SDK would silently skip; unit tests cannot catch malformed live payloads, this signal can.
...
- Test: capture a representative SDK transport log and assert its level and fields (level-mapping sanity, checklist #8).
...
- `internal/mcp-server/server.go` — conditional `WithInputSchemaValidation` (enforce), `WithOutputSchemaValidation`, `WithStreamableHTTPLogger`, enforce-mode validation-rejection telemetry.
