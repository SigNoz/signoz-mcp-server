...
- **Enforce mode** (opt-in strict mode; **not** a future default — see 2026-07-11 reframe): the **same decorator** rejects mismatched input before the handler — the SDK validators (`WithInputSchemaValidation` / `WithOutputSchemaValidation`) are **not used**. One validation owner for all three modes means: rejections are in-band `IsError` results on the repo's coded-error contract (`code: VALIDATION_FAILED`, text names the tool, offending path, and violated constraint), they flow through `loggingMiddleware` like any other tool failure (normal `execute_tool` span/log/metric — exactly one terminal event, no hook glue), and there is no coupling to SDK-pinned message prefixes at all. **Default-flip dropped (owner decision 2026-07-11)**: end users run heterogeneous agents and most do not self-correct from rejection errors — a rejection is a failed turn for them, not a retry. Shadow is therefore the *standing* operating mode, not a phase: its mismatch telemetry drives accept-and-normalize widening (fix the schema or add the normalizer so every agent succeeds), and rejection stays reserved for unrecoverable intent, which handlers already do per-field. Enforce remains available per deployment for CI, integration debugging, and strict single-agent setups. (Post-review simplification 2026-07-10; the earlier design used SDK validation for enforce plus an `AfterCallTool` prefix-detection hook — deleted.)
...
- ~~Enforce-mode default flip~~ **dropped 2026-07-11** (see C): shadow is the standing mode; mismatch telemetry drives normalization/widening fixes instead. Enforce stays opt-in per deployment.
...
10. **Client compat (enforce mode, opt-in)**: rejection message shape differs from current handler-specific error text; relevant only to deployments that opt into enforce (default flip dropped — see C).
