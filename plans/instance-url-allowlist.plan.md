...
This adds an opt-in env-var allowlist of permitted SigNoz instance hostnames. When set,
only matching hosts are accepted; when unset, behavior is unchanged.

## Approach

### Matching (`pkg/util/allowlist.go`)
- `InstanceURLAllowlist` holds a set of exact lowercased hostnames plus a list of
  dot-anchored suffixes (from `*.suffix` patterns).
- `ParseInstanceURLAllowlist(raw string)` splits on `,`, trims, lowercases,
...
- Const `InstanceURLAllowlistEnv = "SIGNOZ_INSTANCE_URL_ALLOWLIST"`.
- `Config.InstanceURLAllowlist util.InstanceURLAllowlist`, parsed in `LoadConfig`.
...
- Helper `enforceInstanceURLAllowlist(ctx, w, r, signozURL, authMode) bool` —
  returns true if allowed; else `logAuthFailure(... 403 ...)`, writes 403, false.
- Call it in the OAuth-token branch (after the SigNoz URL is set in ctx) and in the
...
- After `NormalizeSigNozURLFormInput` succeeds, reject with a 403 authorize page
  (`access_denied`) when `h.config.InstanceURLAllowlist.AllowsURL(normalizedURL)`
...
- In `issueTokenPair` (shared by the authorization_code and refresh_token
  ...
  token-issued analytics, so existing refresh tokens for now-disallowed instances
...
- `plans/instance-url-allowlist.*` — this pair.
