...
2. Normalize the upstream hour/day response variants into one stable hours-based output containing current retention, change status, and active custom log rules. When a v2 pending or failed response does not expose the active policy, report the current state as unknown rather than publishing the attempted configuration.
