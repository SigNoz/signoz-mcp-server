...
### 2026-09-26 — Refresh old PR #263 against current main

Re-read issue #175, original PR #260, PR #263, current code/contracts, and released
SigNoz v0.143.0 backend/OpenAPI/UI sources. All four runtime/schema findings remain
necessary, but unconditional one-query cardinality would reject current TextPanels.
Apply zero queries for TextPanel and exactly one outer wrapper for other panels;
preserve nested CompositeQuery entries, current SDK behavior, canonical `id`, area
charts, TextPanel normalization, and dry-run guidance. Keep the release-pinned
schema generator and heatmap exclusion. Simplify error formatting and redundant
tests/prose; extend existing e2e lifecycles. The current dashboard-creation skill
still has two blanket one-query statements despite merged agent-skills#77, so a
small companion correction is required. Final verification follows below.

### 2026-09-26 — Refresh verification and cleanup outcome

Go 1.26.0 `make ci READY=1` passed with a clean isolated module cache, including
race, guardrail, Inspector, dual-era conformance, Python style, and ready-PR docs
checks. Focused handler/schema/wire/resource tests passed and all embedded schemas
regenerated byte-for-byte from the pinned release. Corrected the review's e2e
assertion finding: upstream defaults may add nested keys, so compare supplied
values while preserving list lengths and order. Docker is unavailable locally;
hosted e2e remains the live gate. Companion agent-skills#102 corrects stale creating
skill guidance and existing eval expectations; its validation check passes.
The PR records the input tightening as a breaking change with inline recovery and
migration guidance. No MUST exceptions; EVL-1 model A/B checks deferred because the
scoped contract behavior is covered by source, deterministic tests, and lifecycle
extensions. Issue #175 remains open until the MCP PR merges.
