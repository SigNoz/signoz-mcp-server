...
- Run the tranche's guardrails in a dedicated, unprivileged `guardrails / contract` pull-request workflow for both internal and fork PRs. Give every guarded test a `TestGuardrail_` prefix and pin the exact discovered test inventory in `guardrails/tests.txt` so additions, removals, and renames require an explicit central-policy diff before the suite runs.
